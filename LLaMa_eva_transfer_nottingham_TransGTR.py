...
file_path = "./xRAG4TS/generated_dataset_transfer.txt"
...
pipe = pipeline(task="text-generation", model=model, tokenizer=tokenizer, max_length=1200)
instruction_text = 'Role: You are an AI agent responsible for predicting the availability of parking lots. Objective: Your task is to forecast the number of available parking lots for the next 3 hours. To do this, you will analyze data from the past 3 hours of parking records, along with simulation-based predictions for the next 3 hours and other additional information that could affect the parking lots availability prediction values. Input Data: (1) Car park description: {}. (2) Prediction horizon: you are given 3 hours historical input from {} to {} on {}, and you are supposed to predict future parking lots from {} to {}. (3) The Natural patterns of parking availability (e.g., peak and off-peak periods throughout the day) are as follow: {}. (4) Historical Records: Parking data for the past 3 hours {}. (5) Simulation predictions: Forecasts for parking availability for the next 3 hours based on simulation models {}. Consider all provided information (historical records, simulation predictions, and additional factors) to predict parking lot availability for the next 3 hours with accuracy. Please analyze the data and provide only your final numerical predictions. DO NOT include any additional content in your answer. Strictly enclose your answer inside square brackets [] and provide exactly 12 numerical values.'
parking_data = pd.read_hdf('./data/nottingham11.h5')
true = np.load('ytrue_transfer_Nottingham.npy')
pred = np.load('ypred_transfer_Nottingham.npy')
carpark_des_list = []
...
test_pred = []
for j in range(11):
    for i in range(2162): ###2163 is the testset num  ### 0~2151 for onefitsall
        t1 = parking_data.iloc[8655-12+i:8655+i,0].index[0]
        t2 = parking_data.iloc[8655-12+i:8655+i,0].index[-1]
        t3 = parking_data.iloc[8655+i:8655+12+i,0].index[0]
        t4 = parking_data.iloc[8655+i:8655+12+i,0].index[-1]
        weekday = parking_data.iloc[8655+i:8655+12+i,0].index[0].day_name()
        Historical = parking_data.iloc[8655-12+i:8655+i,j].values
        prediction = pred[i,j,:]
        truth = true[i,j,:]
        carpark_des = carpark_des_list[j]
        reasonale_hint = reasonale_hint_list[j]
        sample_prompt = instruction_text.format(carpark_des,t1, t2, weekday, t3, t4, reasonale_hint, Historical, prediction)
...
for i in tqdm(range(len(loaded_list))):
    ...
    # if i in [19-1, 1900-1, 672*19-1]:
    #     filename = "answer_list_{}.json".format(i+1)
    #     with open(filename, "w") as file:
    #         json.dump(answer_list, file)

# Open a file in write mode
# print(answer_list[0])
with open("answer_list_transfer_nottingham.json", "w") as file:
